...
class Yampee_Yaml_Parser
{
		...
		while ($this->moveToNextLine()) {
			...
			if (preg_match('#^\-(\s+(?P<value>.+?))?\s*$#u', $this->currentLine, $values)) {
				...
				if (!isset($values['value']) || '' == trim($values['value'], ' ') || 0 === strpos(ltrim($values['value'], ' '), '#')) {
					...
					$parser = new Yampee_Yaml_Parser($c);
					$parser->refs =& $this->refs;
					$data[] = $parser->parse($this->getNextEmbedBlock());
				} else {
					if (preg_match('/^([^ ]+)\: +({.*?)$/u', $values['value'], $matches)) {
						$data[] = array($matches[1] => Yampee_Yaml_Inline::load($matches[2]));
					} else {
						$data[] = $this->parseValue($values['value']);
					}
				}
			} elseif (preg_match('#^(?P<key>'.Yampee_Yaml_Inline::REGEX_QUOTED_STRING.'|[^ \{\[].*?) *\:(\s+(?P<value>.+?))?\s*$#u', $this->currentLine, $values)) {
				$key = Yampee_Yaml_Inline::parseScalar($values['key']);
				...
				if (2 == count($this->lines) && empty($this->lines[1])) {
					$value = Yampee_Yaml_Inline::load($this->lines[0]);
	...
	protected function parseValue($value)
	{
		...
		if (preg_match('/^(?P<separator>\||>)(?P<modifiers>\+|\-|\d+|\+\d+|\-\d+|\d+\+|\d+\-)?(?P<comments> +#.*)?$/', $value, $matches)) {
			...
			return Yampee_Yaml_Inline::load($value);
		}
	}
